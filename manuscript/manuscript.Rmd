...
(ref:overall) Evolution of confirmed Covid-19 cases in Ireland alongside with school closures periods in grey areas (SC).
...
df_school_closure <- df_school |> 
  filter(schools_closed == 1) |> 
  count(schools_closure_wave) |> 
  rename(schools_closure_wave_n = n) |> 
  right_join(df_school, by = "schools_closure_wave") |>
  filter(schools_closure_day == 1) |>
  select(schools_closure_start_date = date, schools_closure_wave, schools_closure_wave_n) |> 
  mutate(
    schools_closure_end_date = schools_closure_start_date + schools_closure_wave_n,
    schools_closure_name = glue::glue("SC {schools_closure_wave}"),
    x_text = schools_closure_start_date + (schools_closure_end_date - schools_closure_start_date)/2
    )
```
...
overall_plot <-
  ggplot() +
  geom_rect(data = df_school_closure, aes(xmin = schools_closure_start_date, xmax = schools_closure_end_date, ymin = -Inf, ymax = Inf), alpha = 0.25) +
  geom_text(data = df_school_closure, aes(label = schools_closure_name, x = x_text, y = Inf), vjust = 2, family = "serif", fontface = "bold", size = 3) +
  geom_line(data = df, aes(date, total_cases), na.rm = TRUE) +
...
```{r te, fig.width=4.5, fig.height=4.5, fig.cap="(ref:te)", fig.pos = "!h"}
te_plot
```
