...
df_global <- df |> 
  ...
  mutate(
    ...
    daily_cases = cumulative_cases_cal - lag(cumulative_cases_cal)
  ) |> 
  ungroup() |> 
...
df_global_na <- df_global |> 
  group_by(country) |> 
  mutate(
    na_or_not = if_else(is.na(cumulative_cases_cal), 1, 0),
    change_seq = na_or_not - lag(na_or_not)
  ) |> 
  select(date, country, change_seq) |> 
  drop_na(change_seq) |> 
  filter(change_seq != 0) |> 
  mutate(time_length = date - lag(date))
...
df_global_school_age_long <- df_global_school_length |> 
  ...
  filter(schools_closure_wave_n >= days_min_closure & schools_time_since_closure <= days_after_closure) |>
...
df_global_school_age_long_euro <- df_global_school_age_long |> 
  filter(
    who_region == "EURO"
  )
...
df_france_check <- df_global_school |> 
...
df_global_school_age_long_euro |> 
  ggplot(aes(schools_time_since_closure, daily_cases, color = country)) +
  facet_grid(age_grp ~ schools_closure_wave) +
  geom_line() +
  theme_bw() +
  theme(legend.position = "bottom")

# analyses ---------------------------------------------------------------------
sink("test.txt", append = FALSE, split = TRUE)

system.time(gam_model <- 
              bam(
                daily_cases ~ age_grp + 
...
system.time(gam_model <- 
              bam(
                daily_cases ~ age_grp * country +
                  s(schools_time_since_closure, by = age_grp) +
                  s(schools_time_since_closure, schools_closure_wave, country, bs = "fs"),
                data = subset(df_global_school_age_long, daily_cases > 0),
                family = "poisson",
                method = "fREML",
                control = list(nthreads = parallel::detectCores())
              )); summary(gam_model)
...
gam_model <- 
  bam(
    cumulative_cases_cal ~ age_grp + country +
      s(schools_time_since_closure, by = age_grp) +
